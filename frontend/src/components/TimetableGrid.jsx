...
export default function TimetableGrid({ timetable, sections, periodsPerDay, daysPerWeek, filterSectionId }) {
  const days = DAYS.slice(0, daysPerWeek);
  const periods = periodsPerDay;

  const sectionList = filterSectionId
    ? sections.filter(s => s.id === filterSectionId)
    : sections;

  if (!timetable || sectionList.length === 0) return null;
  ...
  return (
    <div className={styles.wrap}>
      <table className={styles.table}>
        ...
        <tbody>
          {sectionList.map(sec => {
            const secGrid = timetable[sec.id] || {};
            return days.map((day, dayIdx) => {
              const row = secGrid[day] || [];
              return (
                <tr key={`${sec.id}_${day}`} className={dayIdx % 2 === 0 ? styles.row_even : ''}>
                  {dayIdx === 0 && (
                    <td className={styles.sec_cell} rowSpan={days.length}>
                      {sec.name || sec.id}
                    </td>
                  )}
